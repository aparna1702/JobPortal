...
// import { uploadResumeToCloudinary } from "../middlewares/resumeUpload.js";
...
//         const { resume } = req.files;
//         try {
//             const cloudinaryResponse = await uploadResumeToCloudinary(resume.tempFilePath);
...
//         atsScore = calculateATSScore(resumeText, jobDescription);
//     } catch (err) {
//         console.warn("Failed to parse resume for ATS score. Proceeding with score = 0");
//     }
...
export const postApplication = catchAsyncErrors(async (req, res, next) => {
    ...
    try {
        ...
        const Qualifications = jobDetails.qualifications || jobDetails.description || "";
        atsScore = calculateATSScore(resumeText, Qualifications);
    } catch (err) {
        console.warn("Failed to parse resume for ATS score. Proceeding with score = 0");
    }
